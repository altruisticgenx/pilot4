        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 space-y-4">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <select value={status} onChange={e => setStatus(e.target.value)} className="border border-input rounded-md px-3 py-2 bg-background h-10 min-w-[200px]">
            ...
            {allTags.length > 0 && <div className="flex flex-wrap gap-2 items-center">
                <span className="text-sm text-muted-foreground">Tech tags across experiments:</span>
                {allTags.slice(0, 8).map(tag => <Badge key={tag} variant="secondary" className="text-xs">
                    {tag}
                  </Badge>)}
                {allTags.length > 8 && <span className="text-xs text-muted-foreground">+{allTags.length - 8} more</span>}
              </div>}
          </div>
          ...
          {isLoading ? <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            </div> : experiments && experiments.length > 0 ? <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {experiments.map(exp => {
            const metrics = exp.metrics as Record<string, any> || {};
            return <Card key={exp.id} className="border-2 hover:border-primary/30 transition-all hover:shadow-lg group">
                    <CardHeader>
                    ...
                    <CardContent className="space-y-4">
                      {/* Tags */}
                      {exp.tags && exp.tags.length > 0 && <div className="flex flex-wrap gap-1">
                          {exp.tags.slice(0, 4).map(tag => <Badge key={tag} variant="secondary" className="text-xs">
                              {tag}
                            </Badge>)}
                        </div>}

                      {/* Metrics preview */}
                      {Object.keys(metrics).length > 0 && <div className="grid grid-cols-2 gap-2 text-sm">
                          {Object.entries(metrics).slice(0, 2).map(([key, value]) => <div key={key} className="bg-muted/50 rounded p-2">
                      ...
                      <div className="flex gap-2 pt-2">
                        <Button variant="outline" size="sm" className="flex-1" onClick={() => handleViewDetails(exp)}>
                          View Details
                        </Button>
                        {exp.demo_url && <Button variant="ghost" size="sm" asChild>
                            
                          </Button>}
                        {exp.repo_url && <Button variant="ghost" size="sm" asChild>
